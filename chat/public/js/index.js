  ...
  function updateParticipants(participants) {
   ...
   for (var i = 0; i < participants.length; i++) {
      $('#participants').append('<span id="' + participants[i].id + '">' +
       ...
       nameTable[participants[i].id] = participants[i].name;
    }
  }
...
/*
  ...
  */
  socket.on('nameChanged', function (data) {
    ...
    var message = "has changed their name to";
    var name = data.name;
    $('#messages').prepend('<b>' + nameTable[data.id] + '</b> has changed their name to <b>' + data.name + '</b></br>');
    nameTable[data.id] = data.name;
  });
  ...
  */
  socket.on('incomingMessage', function (data) {
    ...
    var name = data.name;
    $('#messages').prepend('<b>' + name + '</b><br>' + message + '</br>');
  });
